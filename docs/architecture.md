...
> 更新：2026-09-26 · 架构依据：`426a73d` 的源码与契约；新增工作台实现见 service/、web/ 和 scripts/menu.py。部署结果须另行验证。
>
...
这里的“服务端视角”指 **CG 的供给与维护侧**，不表示必须有一台在线服务器。可以通过 Git 和本地安装器完成分发；本仓新增的可选协作工作台实现见 [运行说明](collaboration-workbench.md)。之前下游观察到的菜单部署与这个实现独立，不共享票据、令牌或数据。
...
`service/api.mjs` 提供清单、提交、幂等回执、票据、维护处理和执行证据；`web/` 提供整体图与双视角操作入口；`scripts/menu.py` 提供标准库客户端与离线重交。独立服务使用自己的 SQLite 保存记录，CG 管理会话和每个 harness 的项目令牌分别授权。接口核心不读取模型平台身份，同项目可以跨 harness 接力。包由发布源构建、校验后下载安装。详见 [工作台使用与权限](collaboration-workbench.md)、[独立部署](self-hosting.md) 和 [多 harness 契约](harness-api.md)。

工作台没有替项目执行业务判断或自动升级。它与旧菜单部署的接口观察是两份证据。下文保留的是旧部署的历史观察。

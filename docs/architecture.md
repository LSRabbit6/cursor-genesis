...
`service/api.mjs` 提供清单、提交、幂等回执、票据、维护处理和执行证据；`web/` 提供整体图与双视角操作入口；`scripts/menu.py` 提供标准库客户端与离线重交。SQLite / 平台数据库保存记录。包由发布源构建、校验后下载安装。详见 [工作台使用与权限](collaboration-workbench.md)。

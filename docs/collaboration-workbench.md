...
打开终端显示的本机地址，默认 `http://127.0.0.1:4317`。数据库保存在被 Git 忽略的 `.cg-menu/workbench.sqlite`。可通过 `PORT`、`CG_DB`、`PYTHON` 指定端口、数据库位置和 Python 路径。本地服务只监听回环地址，验证 Host / Origin，拒绝来访者伪造的身份；同源浏览器使用本地维护身份。

独立版使用 CG 自己的管理密钥和短期浏览器会话，harness 使用项目令牌。默认单个维护空间，每个项目可签发多个独立令牌，不依赖任何模型平台身份。CG 不接受来访者或代理声明的用户身份。完整运行方式见 [独立部署](self-hosting.md)，跨 harness 接力见 [HTTP 接入契约](harness-api.md)。
...
**harness 连接自己的 CG 根地址**，使用项目令牌即可走完整链路，不需要网页登录或模型平台账号。网页为每个 harness 单独创建令牌并填写接入名称。曾发布的私有 Sites 地址保留为历史实例，不是独立版地址，不能给通用客户端使用；本地与旧线上数据库不隐式同步。
...
| 接口 | 用途 | 权限 |
|---|---|---|
| `GET /v1/checklist`、`GET /v1/packs` | 清单、模板、发布包目录 | 服务公开读取，不含项目记录 |
| `POST /v1/requests` | 自查或反馈；带 `client_request_id` | CG 管理会话或指定项目令牌 |
| `GET /v1/tickets` | 搜索、项目 / 状态筛选、每页 50 条 | 仅自己可见记录；令牌再限项目 |
| `GET /v1/tickets/:id` | 回执内容、当前状态、历史 | 同上 |
| `POST /v1/tickets/:id` | 维护状态、关联包，或撤回 | 维护操作仅 CG 管理会话；项目令牌可撤回自己的项目请求 |
| `POST /v1/tickets/:id/evidence` | 追加本地执行证据 | 同项目读取权限 |
| `POST /v1/tokens`、`POST /v1/tokens/:id/revoke` | 创建 / 撤销项目令牌 | CG 管理会话 |

所有 SQL 使用参数绑定。多表写入在同一事务中完成。状态更新带 revision，旧页面不能静默覆盖别人刚保存的处理。令牌只存摘要，原文只返回一次。SQLite 迁移由启动入口按迁移名记录，应用请求不修改表结构。
...
独立登录、会话撤销、跨项目拒绝、同项目多 harness 接力和不信任平台头的验证见 `service/tests/standalone.test.mjs`。Docker 镜像已在本机 Linux ARM64 容器中验证构建、非 root 启动、登录、提交与重启恢复；未迁移线上实例。
